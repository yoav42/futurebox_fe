...
export default function CreateChild() {
	...
	const [relation, setRelation] = React.useState("recipient");
	...
	async function onSubmit(e: React.FormEvent) {
		e.preventDefault();
		if (!me) return;
		await api<{ id: string }>("/api/recipients", {
			method: "POST",
			headers: { authorization: `Bearer ${token}` },
			body: JSON.stringify({ parent_id: me.id, full_name: fullName, date_of_birth: dob, relation, bc_last4: bc4, passport_last4: pp4 }),
		});
		setMsg("Recipient created");
		setFullName(""); setDob(""); setBc4(""); setPp4("");
		const list = await api<any[]>("/api/recipients", { headers: { authorization: `Bearer ${token}` } });
		setChildren(list);
	}
    return (
        <div className="max-w-2xl mx-auto">
            <div className="mb-6">
                <h2 className="text-2xl font-semibold">Create recipient (FIR)</h2>
                <p className="text-slate-600">Add a Future Identity Record for your recipient.</p>
            </div>
            {!token ? (
                ...
                <form onSubmit={onSubmit} className="rounded-xl border border-slate-200 bg-white shadow-sm p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input value={relation} onChange={(e) => setRelation(e.target.value)} placeholder="recipient" required className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <ul className="p-3 divide-y divide-slate-200">
                    ...
                    {children.length === 0 && <div className="px-3 py-2 text-sm text-slate-500">No recipients yet.</div>}
                </ul>
            </div>
        </div>
    );
